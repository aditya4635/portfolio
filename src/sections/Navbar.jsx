...
const Navbar = () => {
  ...
  return (
    <header className="fixed top-4 left-0 right-0 z-50 px-4 sm:px-6 lg:px-8">
      <div className="flex justify-between items-center py-3 px-4 sm:px-5 bg-white/80 dark:bg-black-200/80 backdrop-blur-md border border-black-200/10 dark:border-white-800/10 rounded-2xl shadow-lg shadow-violet-500/10">

        {/* Logo */}
        <a href="/" className="text-black dark:text-white font-bold text-lg sm:text-xl hover:text-violet-500 transition-colors flex items-center gap-1.5 flex-shrink-0">
          <span className="text-violet-500 text-xl sm:text-2xl">&lt;</span>
          Aditya
          <span className="text-cyan-500 text-xl sm:text-2xl">/&gt;</span>
        </a>

        {/* Right side controls */}
        <div className="flex items-center gap-2">

          {/* Desktop-only: LeetCode & GitHub — hidden on mobile and tablet */}
          <a
            href="https://leetcode.com/u/adityarajbhandari1020/"
            target="_blank"
            rel="noopener noreferrer"
            className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg font-medium text-sm text-white bg-gradient-to-r from-orange-500 to-yellow-500 hover:from-orange-600 hover:to-yellow-600 transition-all shadow-md"
          >
            LeetCode
          </a>

          {/* Resume — visible from sm (tablet) upward */}
          <a
            href="/assets/NSUT_TnP_Resume (5).pdf"
            download="Aditya_Raj_Bhandari_Resume.pdf"
            className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg font-medium text-sm text-white bg-violet-500 hover:bg-violet-600 transition-all shadow-md"
          >
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg font-medium text-sm text-white bg-gray-800 hover:bg-gray-900 dark:bg-gray-700 dark:hover:bg-gray-600 transition-all shadow-md"
          >
        ...
        <nav className="mt-2 p-5 bg-white/95 dark:bg-black-200/95 backdrop-blur-lg rounded-2xl border border-black-200/10 dark:border-white-800/10 shadow-lg">
          ...
          <div className="sm:hidden mt-4 pt-4 border-t border-black-300/20 dark:border-white-800/10">
            <a
              href="/assets/NSUT_TnP_Resume (5).pdf"
              download="Aditya_Raj_Bhandari_Resume.pdf"
              className="flex items-center justify-center gap-2 w-full px-4 py-2.5 rounded-lg font-medium text-sm text-white bg-violet-500 hover:bg-violet-600 transition-all"
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4">
                <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
              </svg>
